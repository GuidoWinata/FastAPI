...
# Membuat akses mendapatkan Token
...
# async def create_new_user(create_user: createUsers, db: Session = Depends(get_db)):
...
def get_user_exceptions():
    ...
    return credentials_exeptions

def token_exceptions():
    token_exceptions_response = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Incorrect username or password",
        headers={"WWW-Authenticate":"Bearer"}
    )
    return token_exceptions_response